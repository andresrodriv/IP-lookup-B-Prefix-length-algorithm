...
class SimpleSwitch13(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		wsgi = kwargs['wsgi']
		self.algorithm = input("Which algorithm do you want to use: ")
...
##############################GENERATION OF THE TRIE###################################################################
			...
			for i in self.switch:
				...
				binIP = str((bin(int(ipaddress.IPv4Address(unicode(i))))))[
						2:]  # unicode for the IPv4 Address formatting
...
# -------------------------------------------------------------------------------------------------------
	...
	def linear_search(self, dst_ip):
		...
		if dst_ip in self.lookup:
			...
			self.logger.info("Next Hop Returned: %s", destination_switch_IP)
			return destination_switch_IP
		else:			destination_switch_IP = "0"
			self.logger.info("Next Hop Returned: %s", destination_switch_IP)
			return destination_switch_IP
